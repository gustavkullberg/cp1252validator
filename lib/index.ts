...
export const hasCharacterNotInCP1252 = (input: string) => {
  try {
    if (input !== decodeURIComponent(input)) return regex.test(decodeURIComponent(input));
  } catch (e) {}
  return regex.test(input);
};

export const textIsCP1252Compatible = (input: string) => {
  try {
    if (input !== decodeURIComponent(input)) return !regex.test(decodeURIComponent(input));
  } catch (e) {}
  return !regex.test(input);
